...
- **Main server:** Raspberry Pi 4 (4 GB) in a Geekworm SATA case, 2 TB SSD
  in the bay. Boots straight from the SSD (GPT; boot order `0xf14` = USB
  first). An SD card sits in the slot as an automatic rescue system — a
  standalone bootable OS that only kicks in if the SSD doesn't.
- **ML server:** second Pi 4 (4 GB), `pi-ml`, runs from a 32 GB SD card.
  Only Docker plus the Immich ML container.
...
- **Samba** (main server): `[backup-anna]` → `/srv/backup/anna` and
...
- **Immich** (`/srv/docker/immich`, port 2283): phone uploads with storage
  ...
  Postgres capped at `shared_buffers=256MB`. The compose file is
  **generated** by `transform-compose.py` from the upstream template — local
  changes belong in that script, never in the compose file itself.
- **Immich ML** (`pi-ml:3003`, `/srv/docker/immich-ml`): CLIP text model
  ...
  after 1 h idle. ML URL set in the Immich admin settings. Version pinned to
  the same major tag as the server.
- **Updates:** unattended-upgrades on both Pis (auto-reboot 04:00);
  ...
  a month, updates both Pis, all containers, rebuilds the Immich compose,
  checks SMART, probes all services. The ML Pi is reached through a
  dedicated SSH key that is locked to a single forced command
  (`ml-update.sh`), backed by a sudoers exception for exactly that script.
- Docker log rotation (10 MB × 3) and memory cgroups active on both Pis.
...
- **The main server's LAN IP works everywhere**: at home directly (DHCP
  ...
  **subnet router** for `192.168.0.0/24` (route approved in the admin
  console; no exit node).
- Services: `http://192.168.0.10:2283` (Immich), `:8000` (Paperless —
  `PAPERLESS_ALLOWED_HOSTS` contains LAN and tailnet IP), `smb://…`.
- MagicDNS unused on Android (private DNS wins) — access by IP.

## Security

- SSH: **password login only** (no stored keys), sudo requires a password.
  The single permanent exception: the restricted main→ML key described
  above, which cannot open a shell.
- Tailscale: least-privilege **ACL** (laptops full access, phone only
  reaches the three services on the main server), **device approval** on,
  key expiry disabled **only** for the server.
