...
| [Container with its own Tailscale node](home-server/container-tailscale-node.md) | Giving one Docker service its own tailnet identity, so node sharing exposes only that service: container Tailscale, shared network namespace, no host ports |
